    ...
    <form class="form-horizontal" role="form" action="/adminusers" method="post"> 
     <div class="form-group" style="margin-top: 40px"> 
      <label class="col-lg-2 col-sm-2 control-label" for="inputEmail1">管理员名:</label> 
       ...
       <input class="form-control" name="name" id="inputEmail1" type="text" placeholder="必须输入4-8位数字或字母" value="{{old('name')}}"/> 
      </div> 
     </div> 
     ...
     <div class="form-group" style="margin-top: 40px"> 
      ...
      <div class="col-md-3 col-xs-9"> 
          <select class="form-control m-bot15" name="rid">
            @foreach($res as $row)
            <option value="{{$row->id}}" @if($row->name == '一级管理员') selected @endif>{{$row->name}}</option>
            @endforeach 
            </select> 
      </div> 
      </div>
